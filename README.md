...
| Terminal | Alacritty |
| Status Bar | swaybar (native) + sway-status |
| App Launcher | tofi |
| Audio | PipeWire |
| Fonts | Inconsolata |
...
- **Solarized Dark everywhere** — consistent palette across Alacritty, swaybar, and tofi.
...
- **Terminal font is smaller than the rest of the UI** — Alacritty runs at 12pt Inconsolata; Sway's titlebars, tofi, and the swaybar status line are all 20pt Inconsolata (Pango points) — the same literal font directive throughout, now that swaybar is native Sway UI rather than a separate GTK app with its own CSS sizing.
...
- **The current mode is exposed for the status bar** — `sway-masterstack status` prints `[]=` or `[M]` for the focused workspace, and `sway-masterstack title` prints the focused window's title; `sway-status` shells out to both as the first two segments of its status line. The daemon signals `sway-status` (`pkill -SIGUSR1 -x sway-status`) on every mode change and workspace switch instead of it polling on a timer — the same feedback dwm's own bar gives for free.
...
│   └── sway-status                  # swaybar status line script
├── .config/
│   ├── sway/config                  # Window manager + status bar
